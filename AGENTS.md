...
**참고:** `ServerLib/Core/Serialization/Packets/`에는 삭제된 데모가 쓰던 패킷 타입(Damage·Mob·Ticket·Seat·Login·AuthToken·Increment·Decrement·CounterQuery·CounterValue 등)이 남아 있다. 라이브러리 코어와 무관한 데모 프로토콜이므로, 추가 정리 시 `EchoPacket`·`PingPacket`·`PongPacket`만 남기고 제거를 검토할 수 있다.
